...
def conv2d_forward(x, w, b, pad, stride):
    ...
    new_height = ((height - filter_height + 2 * pad) // stride) + 1
    new_width = ((width - filter_width + 2 * pad) // stride) + 1
    output = np.zeros((x.shape[0], new_height, new_width, w.shape[0]))
    x_padded = np.zeros((x.shape[0], x.shape[1] + 2 * pad, x.shape[2] + 2 * pad, x.shape[3]))
    x_padded[:,1:-1, 1:-1, :] = x
    for n in range(x.shape[0]):
        for m in range(w.shape[0]):
            for i in range(output.shape[1]):
                for j in range(output.shape[2]):
                    output[n,i,j,m] = (w[m] * x_padded[n, i*stride:i*stride+filter_height,\
                     j*stride:j*stride+filter_width] + b).sum()
