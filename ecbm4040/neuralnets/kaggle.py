...
def my_LeNet(input_x, input_y,
    ...
    # fc_layer_middle = tf.layers.dropout(fc_layer_0.output())

    # fc_layer_1 = fc_layer(input_x=fc_layer_middle,
    #                       in_size=fc_units[0],
    #                       out_size=output_size,
    #                       rand_seed=seed,
    #                       activation_function=None,
    ...
    # fc_layer_1 = tf.layers.dropout(fc_layer_0.output())
    ...
    with tf.name_scope("loss"):
        ...
        tf.summary.scalar('myLeNet_loss', loss)
...
def train_step(loss, learning_rate=1e-3):
    with tf.name_scope('train_step'):
        # step = tf.train.GradientDescentOptimizer(learning_rate).minimize(loss) 10 epoches, 21.7%
        step = tf.train.AdamOptimizer(learning_rate).minimize(loss) #10 epoches, 53.5%, 20 epoches, 57.2%
...
def evaluate(output, input_y):
    with tf.name_scope('evaluate'):
        ...
        tf.summary.scalar('myLeNet_error_num', error_num)
    return error_num
...
def kaggle_training(X_train, y_train, X_val, y_val, 
    ...
    print("Building my myLeNet. Parameters: ")
